...
from src.utils.data import UnseededDataLoader, SeededDataLoader
...
def get_data_tuples(folder_path: str, n_elements: int = -1) -> List[Dict[str, List[Union[str, Tuple[str, Optional[str]]]]]]:
    """
    ...
    folder_path = folder_path.split('.')[0] if folder_path.endswith('.zip') else folder_path
    
    # Check that in each intact and dameged folder, there are three folders: pre, post and mask
    for subfolder in ['pre', 'post', 'mask']:
        assert os.path.exists(os.path.join(folder_path, subfolder)), f'❌ Folder {os.path.join(folder_path, subfolder)} does not exist.'
...
def get_split_data() -> Tuple[
    ...
    train_data = damaged[:int(TRAIN_SPLIT*len(damaged))] + intact[:int(TRAIN_SPLIT*len(intact))]
    test_data = damaged[int(TRAIN_SPLIT*len(damaged)):int((TRAIN_SPLIT+TEST_SPLIT)*len(damaged))] + intact[int(TRAIN_SPLIT*len(intact)):int((TRAIN_SPLIT+TEST_SPLIT)*len(intact))]
    val_data = damaged[int((TRAIN_SPLIT+TEST_SPLIT)*len(damaged)):] + intact[int((TRAIN_SPLIT+TEST_SPLIT)*len(intact)):]
    
    return train_data, test_data, val_data

def get_dataloaders() -> Tuple[SeededDataLoader, UnseededDataLoader, UnseededDataLoader]:
    """
    Get the train, test and val dataloaders.

    Returns:
        Tuple[SeededDataLoader, UnseededDataLoader, UnseededDataLoader]: Train, test and val dataloaders
    """
    ...
    train_dataloader = SeededDataLoader(SEED, train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS)
    test_dataloader = UnseededDataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=1)
    val_dataloader = UnseededDataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=1)
