...
from src.utils.sweeper import Sweeper
...
sweep_config = {
    ...
    'parameters': {
        ...
        'weight_decay': {
            'distribution': 'categorical',
            'values': [1e-1, 1e-2, 1e-3, 1e-4, 0],
        },
        'siamese_embedding_size': {
            'distribution': 'categorical',
            'values': [16, 32, 64, 128],
        },
    }
...
def train(config=None):
    ...
    sweeper.train(train_dataloader, val_dataloader)

if __name__ == '__main__':
    sweep_id = wandb.sweep(sweep_config, project="siamese-network")
