...
def _get_patch_files() -> List[str]:
    # Get all patch files
    pre_patch_files = [
        f"{MAXAR_PRE_PATCHES_PATH}{file}" for file in os.listdir(MAXAR_PRE_PATCHES_PATH)
    ]
    post_patch_files = [
        f"{MAXAR_POST_PATCHES_PATH}{file}"
        for file in os.listdir(MAXAR_POST_PATCHES_PATH)
    ]
    print(len(pre_patch_files), len(post_patch_files))

    # Get short file names
    short_pre_patch_files = set(
        _get_short_patch_file_name(file) for file in pre_patch_files
    )
    short_post_patch_files = set(
        _get_short_patch_file_name(file) for file in post_patch_files
    )
    ...
    pre_patch_files = [
        file
        for file in pre_patch_files
        if _get_short_patch_file_name(file) in same_patch_files
    ]
    post_patch_files = [
        file
        for file in post_patch_files
        if _get_short_patch_file_name(file) in same_patch_files
    ]
