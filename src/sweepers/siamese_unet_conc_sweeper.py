...
def train(config: dict = None) -> None:
    ...
    sweeper = Sweeper(model_class=SiameseUNetConc, config=config)
    sweeper.train(train_loader=train_dataloader, val_loader=val_dataloader)


if __name__ == "__main__":
    config_path = f"{GLOBAL_DIR}/config/siamese_unet_conc_sweep_config.yml"
    config = get_config(config_path)

    sweep_id = wandb.sweep(config, project="Siamese-UNet-Conc-Sweep")
